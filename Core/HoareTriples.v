...
Require Import DiSeL.Core.InductiveInv.
...
(* Section Test. *)
(* (* Testing simple programs with non-specified worlds *) *)

(* Variable this : nid. *)
(* Variable W : world. *)
(* Variables (A B : Type). *)
(* Variables (e1 : action W A this). *)
(* Variable e2 : A -> action W B this. *)

(* (* Simple program *) *)

(* Notation loc l i := (getLocal this (getStatelet i l)). *)

(* Variables (p0 p1 : ptr) (l : Label). *)


(* Definition test_spec : spec B := *)
(*   (* Precondition *) *)
(*   (fun i => loc l i = p0 :-> 42 \+ p1 :-> [::1; 2; 3] /\ False, *)
(*   (* Postcondition *) *)
(*   fun i y m => forall i', i' = i -> loc l m = loc l i). *)

(* Definition test_bin :=  DTbin this W test_spec. *)

(* Program Definition test : test_bin := *)
(*   Do (x <-- act e1; *)
(*      act (e2 x)). *)

(* Next Obligation. *)
(* by rewrite /conseq/==>i[E]F. *)
(* Qed.     *)
...
(*   Definition p2spec : spec (seq nat) := *)
(*     (* precondition *) *)
(*     ( fun _ => True *)
(*     (* postcondition *) *)
(*     , fun _ r _ => r = [::42] *)
...
(*   Definition p2bin := DTbin this W p2spec. *)

(*   Variable send : nid -> nat -> action W unit this. *)
(*   Variable recv : action W (seq nat) this. *)

(*   Axiom sorry : False. *)
(*   Ltac sorry := destruct sorry. *)

(*   Program Definition p2 : p2bin := *)
(*     Do (act (send them 42);; *)
(*         act recv). *)
(*   Next Obligation. *)
(*     rewrite /conseq /verify /=. *)
(*     intros. sorry. *)
(*   Qed. *)
